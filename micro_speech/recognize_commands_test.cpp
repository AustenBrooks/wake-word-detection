...
TF_LITE_MICRO_TEST(RecognizeCommandsTestBasic) {
  ...
  TfLiteTensor results       = tflite::testing::CreateQuantizedTensor(
          result_data, tflite::testing::IntArrayFromInts(result_dims), -128.0f, 127.0f);

  const char *found_command;
  uint8_t     score;
  bool        is_new_command;
  TF_LITE_MICRO_EXPECT_EQ(
    kTfLiteOk, recognize_commands.ProcessLatestResults(&results, 0, &found_command,
...
TF_LITE_MICRO_TEST(RecognizeCommandsTestFindCommands) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    TF_LITE_MICRO_EXPECT_EQ(kTfLiteOk, recognize_commands.ProcessLatestResults(
                                         &yes_results, current_time_ms, &found_command,
                                         &score, &is_new_command));
    if (is_new_command) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    TF_LITE_MICRO_EXPECT_EQ(kTfLiteOk, recognize_commands.ProcessLatestResults(
                                         &no_results, current_time_ms, &found_command,
                                         &score, &is_new_command));
    if (is_new_command) {
      TF_LITE_MICRO_EXPECT(!has_found_new_command);
      has_found_new_command = true;
...
TF_LITE_MICRO_TEST(RecognizeCommandsTestBadInputLength) {
  ...
  TF_LITE_MICRO_EXPECT_NE(
    kTfLiteOk, recognize_commands.ProcessLatestResults(&bad_results, 0, &found_command,
                                                       &score, &is_new_command));
}
...
TF_LITE_MICRO_TEST(RecognizeCommandsTestBadInputTimes) {
  ...
  bool        is_new_command;
  TF_LITE_MICRO_EXPECT_EQ(
    kTfLiteOk, recognize_commands.ProcessLatestResults(&results, 100, &found_command,
                                                       &score, &is_new_command));
  TF_LITE_MICRO_EXPECT_NE(
    kTfLiteOk, recognize_commands.ProcessLatestResults(&results, 0, &found_command,
                                                       &score, &is_new_command));
